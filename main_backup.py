...
def build_knapsack_cqm(profits, vertex1, vertex2,weight, max_weight,P=24):
    ...
    for i in range(1, num_items):            
        constraint1.add_variable('BINARY', "x[{}][{}]".format(0,i))
        constraint1.set_linear("x[{}][{}]".format(0,i), -P)
        # for j in range(i+1, num_items):
        #     print("x[{}][{}]".format(0,i),"x[{}][{}]".format(0,j))
        #     constraint1.set_quadratic("x[{}][{}]".format(0,i),"x[{}][{}]".format(0,j), P)
    
    cqm.add_constraint(constraint1, sense="==", rhs=1, weight=2*P, penalty='quadratic',  label='c1')
    #constraint to end at node n          
    for j in range(num_items-1):            
        constraint2.add_variable('BINARY', "x[{}][{}]".format(j,num_items-1))
        constraint2.set_linear("x[{}][{}]".format(j,num_items-1), -P)
    ...
    for k in range(1, num_items-1):
        for i in range(num_items-1):
            if i != k:            
                ...
                constraint3.set_linear("x[{}][{}]".format(i,k), P)
    ...
    for k in range(1, num_items-1):
        for j in range(1,num_items):
            if j != k:            
                ...
                constraint4.set_linear("x[{}][{}]".format(k,j), P)
    ...
    # print(constraint1.variables, constraint1.linear, constraint1.quadratic)
    # print(constraint2.variables, constraint2.linear)
    # print('Check S3',constraint3.variables, constraint3.linear, constraint3.quadratic)
    # print(constraint4.variables, constraint4.linear)
    # print(constraint5.variables, constraint5.linear)
    print('constrinat' ,cqm.num_biases)
    return cqm
